...
void process_inittab() {
	...
	printf("Checking for /etc/inittab: ");
	fflush(stdout);
...
void generate_mtab() {
	...
	for (j = 0; j < 8; ++j) {
		if (0 == getfsys(j, &info)) {
			...
			if (k == -1) continue;
			if (lseek(k, (long)info.ptr, SEEK_SET) != (long)info.ptr) continue;
			if (read(k, &fsys, sizeof(filesys_t)) != sizeof(filesys_t)) continue;
			if (fsys.s_mounted && fsys.s_mntpt == NULL) {
				j = MINOR(fsys.s_dev);
				unlink("/etc/mtab");
				mtabline[7] = j + '0';
...
int spawn_tty(unsigned num, char **argv) {
	...
	if (uname(&name) == -1) {
		...
		strcpy(name.version, "?.?");
		strcpy(name.machine, "???");
		strcpy(name.domainname, "unknown");
	}
	printf("%c%s operating system at %s\n", 12, name.sysname, name.domainname);
	printf("Kernel %s.%s on a %s\n\n%s ", name.version, name.release, name.machine, name.nodename);
	fflush(stdout);
...
#ifdef NO_RTC
	for (;;) {
		printf("\nClock setup (hh:mm[:ss] dd/mm/yyyy)\n");
