...
int do_top() {
	...
	ptab_t pinfo, ppinfo;
	ptptr p;
	int i, j, delay = 5, pnprocs = 0, securemode = 0, ppid;
	int totalram, usedram = 0, kram, maxdisp = MAXDISP, procsdisp = -1;
	int runonly = 0;
	int useronly = 0;
	int kuid = -1, errv;
	unsigned long secs;
...
again:		home();
		...
		for (p = (ptptr)info.ptr, i = 0; i < info.size; ++i, ++p) {
			...
			errv = getfsys(GI_PDAT, &pdata);
			usedram += pdata.u_size;
			if (runonly && pinfo.p_status != P_RUNNING &&
				pinfo.p_status != P_READY) continue;
			if (useronly && pinfo.p_uid != kuid) continue;
			...
			ppid = -1;
			if (lseek(k, (long)info.ptr, SEEK_SET) == (long)info.ptr) {
				if (read(k, &ppinfo, sizeof(ptab_t)) == sizeof(ptab_t)) {
					ppid = ppinfo.p_pid;
				}
			}			
			/* show info */
			printf("%-5d %-5d %-4d %-4d %-8s %s\t%-4s %3d%c %3d%c %s %s\n",
				pinfo.p_pid, ppid, pinfo.p_cprio, pinfo.p_nice,
				username, mapstat(pinfo.p_status), pssize, pcpu, 
