 ...
 */
static int prtfld __P((FILE *op, unsigned char *buf,
			int ljustf, char sign, char pad,
			int width, int preci, int buffer_mode));

static int prtfld(op, buf, ljustf, sign, pad, width, preci, buffer_mode)
	register FILE *op;
...
nopad:				pad = ' ';
chrpad: 			sign = '\0';
printit:			cnt += prtfld(op, (unsigned char *)ptmp, 
