 ...
 */
static char *reg __P((int paren, int *flagp));
static char *regbranch __P((int *flagp));
static char *regpiece __P((int *flagp));
static char *regatom __P((int *flagp));
static char *regnode __P((char op));
static void regc __P((uint b));
static void reginsert __P((char op, char *opnd));
static void regtail __P((char *p, char *val));
static void regoptail __P((char *p, char *val));
static int regtry __P((regexp *prog, char *string));
static int regmatch __P((char *prog));
static int regrepeat __P((char *p));
static char *regnext __P((char *p));
static char *regprop __P((char *op));
#ifdef STRCSPN
	...
	int *flagp;
{
	...
	*flagp |= flags&SPSTART;
	...
	if (paren && *regparse++ != ')') {
		...
		else	FAIL("junk on end");	/* "Can't happen". */
		/* NOTREACHED */
	}
	return ret;
}
	...
	regexp *r;
{
	...
	while (op != END) {	/* While that wasn't END last time... */
		...
		if (next == NULL)		/* Next ptr. */
			printf("(0)");
		else	printf("(%d)", (s-r->program)+(next-s));
		s += 3;
	...
	char *op;
{
	...
	case OPEN+6: case OPEN+7: case OPEN+8:case OPEN+9:
		...
		p = NULL;
		break;
	case CLOSE+1: case CLOSE+2: case CLOSE+3: case CLOSE+4: case CLOSE+5:
	case CLOSE+6: case CLOSE+7: case CLOSE+8: case CLOSE+9:
