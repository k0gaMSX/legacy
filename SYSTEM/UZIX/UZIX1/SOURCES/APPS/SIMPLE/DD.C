...
#ifdef HI_TECH_C
void doconv(int (*conv)(), int c) {
	if (conv == cnull) cnull(c); else
	if (conv == ebcdic) ebcdic(c); else
	if (conv == ibm) ibm(c); else
	if (conv == ascii) ascii(c); else
	if (conv == block) block(c); else
	if (conv == unblock) unblock(c); else {
		fprintf(stderr, "PANIC: uknown conversion\n");
		exit(1);
	}
}
#endif
...
void usage()          
{
/*  	   printf("convert and copy a file\n\n");*/
