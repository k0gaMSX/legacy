...
void step(char **pcur, char **pnext);
...
int find_page(name, sect)
	char *name;
	char *sect;
{
#ifdef __TURBOC__
	static char defpath[] = "w:/util/manpages";
	static char defsect[] = "1;2;3;4;5;6;7;8;9";
	static char defsuff[] = ";.gz;.Z";
	static char manorcat[] = "man;cat";
#else
	static char defpath[] = "/usr/man:/usr/local/man";
	static char defsect[] = "1:2:3:4:5:6:7:8:9";
	static char defsuff[] = ":.gz:.Z";
	static char manorcat[] = "man:cat";
#endif
...
void do_file() {
	...
	while ((nl = fetch_word()) >= 0) {
		...
		if (catmode) {
			if (strcmp(word, "'\\\"") == 0 || 
			    strcmp(word, "'''") == 0) {
				/* This is a marker sometimes used for opening
				 * subprocesses like tbl and equ; this program
				 * ignores it.
				 */
				do_skipeol();
			}
			...
			else	fprintf(ofd, "%s%s", whitespace, word);
		}
		else {
			if (keep_nl && nl && !no_nl) {
				if (optional_keep) {
					...
					if (line_ptr == 0 || 
					    next_line_indent < 0 ||
					    left_indent + (line_ptr - line) + 1 > 
					      next_line_indent)
						line_break();
					else if (line_ptr != 0 && 
					         next_line_indent > 0) {
						while (left_indent + (line_ptr - line) + 1 <= 
						          next_line_indent)
							*line_ptr++ = cur_font + ' ';
					}
				}
...
int do_fontwords(this_font, other_font, early_exit)
...
void build_headers() {
	...
	for (;;) {
		...
		last_ch = ch;
		if (in_quote || !isspace(ch)) {
			/* Nb, this does nothing about backslashes, 
			 * perhaps it should
			 */
	...
	char	*pword;
{
	...
	*d = 0;
	...
	if (*wword == 0)
		return;
	if (line_ptr) {
		if (line_ptr + (line_ptr[-1] == '.') - line + length >=
	...
	if (find_page(manname, mansect) < 0) {
		if (mansect != 0)
			fprintf(stderr, "No entry for %s in section %s of the manual.\n",
				manname, mansect);
		else	fprintf(stderr, "No manual entry for %s\n", manname);
		return 1;
	}
	if (flg_w)
		return 0;
