...
int tar_fd;
char usage[] = "usage: tar [cxt][voFfpD] tarfile [files]";
...
char *file;
{
...
#if defined(MSX_UZIX_TARGET) || defined(PC_UZIX_TARGET)
  int y=0,mn=0,d=0,hh=0,mm=0;
  unsigned long t1;
  time_t tt;
  
  t1=convert(header.dbuf.mtime, LONG_TYPE);
  if (t1>=31536000) { 
   y=t1/31536000L;
   t1=t1-(y*31536000L);
   y=y-10;
  ...
  tt.t_time=(((int)hh << 8) << 3) | ((int)mm << 5) | ((int)t1 >> 1);
  tt.t_date=(((int)y << 8) << 1) | ((int)mn << 5) | d; 
  buf.modtime = buf.actime = tt;
#else
