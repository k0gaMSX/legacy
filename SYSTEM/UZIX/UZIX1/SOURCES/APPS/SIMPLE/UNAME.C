...
static void print_element (mask, element)
	unsigned char mask;
	char *element;
{
	if (toprint & mask) {
	...
	char **argv;
{
	...
	for (i = 1; i < argc; i++) {
		switch(argv[i][1]) {
		case 's':	toprint |= PRINT_SYSNAME;	break;
		case 'n':	toprint |= PRINT_NODENAME;	break;
		case 'r':	toprint |= PRINT_RELEASE;	break;
		case 'v':	toprint |= PRINT_VERSION;	break;
		case 'm':	toprint |= PRINT_MACHINE;	break;
		case 'a':
			toprint = PRINT_SYSNAME | PRINT_NODENAME |
			PRINT_RELEASE | PRINT_VERSION | PRINT_MACHINE;
			break;
		default:
			write(STDOUT_FILENO,"usage: uname [-amvrns]\n",23);
			return;
		}
	}
	if (toprint == 0)
