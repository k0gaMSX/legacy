...
/* no assertions for MSX UZIX - let's save memory! */
#define NDEBUG
#undef CACHE_ROOT
#endif
...
_PROTOTYPE(void delete, (DIRECTORY *entry) );
_PROTOTYPE(void make_file, (DIRECTORY *dir_ptr, int entries, char *name) );
_PROTOTYPE(void fill_date, (DIRECTORY *entry) );
_PROTOTYPE(char *make_name, (register DIRECTORY *dir_ptr, short dir_fl) );
_PROTOTYPE(int fill, (char *buff, size_t size) );
...
_PROTOTYPE(unsigned short clear_cluster, (unsigned int cl_no) );
...
DIRECTORY *entry;
{
  ...
  time(&atime);
  cur_time = convtime(&atime) - DOS_TIME;
...
unsigned short clear_cluster(cl_no)
register unsigned int cl_no;
{
  unsigned short old;
