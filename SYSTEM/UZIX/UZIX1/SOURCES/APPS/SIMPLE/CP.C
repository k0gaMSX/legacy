...
#if defined(MSX_UZIX_TARGET) || defined(PC_UZIX_TARGET)
#define LINKSIZE	128+1
#else
#define	LINKSIZE	1024+1
#endif
...
#ifdef HI_TECH_C
		if (action == MOVE) {
			_now.t_time=srcst->st_atime.t_time;
			_now.t_date=srcst->st_atime.t_date;
		}
		ut.actime.t_time=_now.t_time;
		ut.actime.t_date=_now.t_date;
		_now.t_time=srcst->st_mtime.t_time;
		_now.t_date=srcst->st_mtime.t_date;
		ut.modtime.t_time=_now.t_time;
		ut.modtime.t_date=_now.t_date;
#else
		ut.actime= action == MOVE ? srcst->st_atime : _now;
...
/* Perform the appropriate action on a source and destination file. */
{
	...
	if (action != REMOVE) {
		...
		/* Copy the file modification time. */
		if (LONG(dstst.st_mtime) != LONG(srcst.st_mtime)) {
			time_t _now;
