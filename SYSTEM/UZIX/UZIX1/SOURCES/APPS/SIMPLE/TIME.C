...
#define diffticks(x, y)		x.t_time+x.t_date*CLOCKS_PER_SEC*60- \
				y.t_time+y.t_date*CLOCKS_PER_SEC*60
#else
#define diffticks(x, y)		x-y
#endif

#ifndef std_err
#define std_err(x)		write(STDOUT_FILENO, x, strlen(x))
#endif
...
char *argv[];
{
  ...
  if (argc <= 1) {
  	std_err("usage: time program [program args]\n");
  	exit(0);
  }
  ...
  if ((status & 0x00ff) != 0) std_err("Command terminated abnormally.\n");
  times(&post_buf);

  /* Print results.  -DNEW_TIME_FORMAT enables time on one line to 0.01 sec. */
#ifndef NEW_TIME_FORMAT
  std_err("real ");
  print_time((clock_t) difftime(&end_time, &start_time) * CLOCKS_PER_SEC);
  std_err("\nuser ");
  print_time(diffticks(post_buf.tms_cutime, pre_buf.tms_cutime));
  std_err("\nsys  ");
  print_time(diffticks(post_buf.tms_cstime, pre_buf.tms_cstime));
  std_err("\n");
#else
  print_time((clock_t) difftime(&end_time, &start_time) * CLOCKS_PER_SEC);
  std_err(" real");
  ...
  std_err(" sys\n");
#endif
  return((status & 0x00ff) ? -1 : (status >> 8));
}
