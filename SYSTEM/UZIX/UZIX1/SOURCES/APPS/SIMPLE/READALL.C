...
#define RESUME  50		/* # good reads before going back to CHUNK */
#define DIVISOR	 50		/* how often to print statistics */
#define STORE  288		/* save this many bad blocks for summary */
...
char *argv[];
{
  ...
  while (1) {
	...
	printf("will read %d blocks from %d sector\n", chunk, b);
	...
	if (s == BLOCK_SIZE * chunk) {
		...
		/* End of file. */
printf("read only %d blocks (%d bytes)\n", s, s / BLOCK_SIZE);
		b += s / BLOCK_SIZE;
printf("last block read is %d\n", b);
		if (normal) {
			output(b);
			fprintf(stderr, "\n");
		}
