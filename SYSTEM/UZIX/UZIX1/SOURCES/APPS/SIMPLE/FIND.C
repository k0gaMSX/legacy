...
register struct node *n;
{
  ...
  switch (n->n_type) {
    ...
    case OP_NEWER:
	return ((convtime(&st->st_mtime) - n->n_info.n_int.n_val) > 0);
    case OP_TYPE:
    ...
    case OP_ATIME:
	return ichk(convtime(&st->st_atime), n);
    case OP_CTIME:
	return ichk(convtime(&st->st_ctime), n);
    case OP_MTIME:
	return ichk(convtime(&st->st_mtime), n);
    case OP_EXEC:
    case OP_OK:
...
int t;
{
  ...
  switch (t) {
	...
	p->n_info.n_int.n_val = convtime(&current_time) - l * SECS_PER_DAY;
	...
	p->n_info.n_int.n_val = convtime(&est.st_mtime);
	break;
    case OP_NAME:
	checkarg(*++ipp);
	p->n_info.n_str = *ipp;
