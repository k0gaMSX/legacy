...
_PROTOTYPE(void outc, (register char c));
...
int main(argc, argv) int argc; char *argv[];
{
	...
	while ((curarg = argv[1]) != NULL && curarg[0] == '-') {
		if (((curarg[1] == 'd') || (curarg[1] == 'D')) &&
			...
			argc--;
			if (argc < 2) {
				format("uudecode: Missing target directory.\n");
				Error(15);
			}
			target = argv[1];
			if (debug)
			...
			argc--;
			if (argc < 2) {
				format("uudecode: Missing source directory.\n");
				Error(15);
			}
			source = argv[1];
			if (debug)
 ...
 */
	loop {	/* master loop for multiple decodes in one file */
...
#endif
		{
			format("uudecode: Missing filename in begin line.\n");
			Error(10);
		}
		...
		if((out = fopen(ofname, WRITE)) == NULL) {
			format("uudecode: Cannot open output file: %s\n", ofname);
			Error(4);
		}
...
again:	if (fgets(buf, sizeof buf, in) == NULL) {
		format("uudecode: EOF while in translation table.\n");
		Error(5);
	}
	numl++;
	if (strncmp(buf, "begin", (size_t)5) == 0) {
		format("uudecode: Incomplete translation table.\n");
		Error(6);
	}
	...
	*cpt = ' ';
	...
	while (c = *cpt) {
		if (chtbl[c] != -1) {
			format("uudecode: Duplicate char in translation table.\n");
			Error(7);
		}
		if (n == 0) blank = c;
		chtbl[c] = n++;
...
void decode()
{
	...
	loop {
		if (fgets(buf, sizeof buf, in) == NULL) {
			format("uudecode: EOF before end.\n");
			fclose(out);
			Error(8);
		}
		numl++;
 ...
 */
		...
		format("uudecode: Bad prefix line %d in file: %s\n",numl, ifname);
 ...
 */
		if (check) {
			if (buf[rlen] != seqc) {
				format("uudecode: Wrong sequence line %d in %s\n",
					numl, ifname);
				if (debug)
					format(
 ...
 */
		...
		if ((n = fwrite(outl, (size_t)1, (size_t)len, out)) <= 0) {
			format("uudecode: Error on writing decoded file.\n");
			Error(18);
		}
	}
}
...
void getfile(buf) register char *buf;
{
	if ((pos = getnword(buf, 2)) == NULL) {
		format("uudecode: Missing include file name.\n");
		Error(17);
	} else
		if (source != NULL) {
			strcpy(ifname, source);
...
#endif
	{
		...
		if (freopen(ifname, "r", in) == in) {
			...
			format("uudecode: Freopen abort: %s\n", ifname);
			Error(9);
		}
	}
	loop {
		if (fgets(buf, LINELEN, in) == NULL) {
			format("uudecode: No begin line after include: %s\n", ifname);
			Error(12);
		}
		numl++;
		if (strncmp(buf, "table", (size_t)5) == 0) {
 ...
 */
	if ((pos = getnword(buf, 3)) == NULL ) {
		format("uudecode: Missing part name, in included file: %s\n", ifname);
		Error(13);
	} else {
		part = *pos;
		partn++;
		if (partn > 'z') partn = 'a';
		if (part != partn) {
			format("uudecode: Part suffix mismatch: <%c> instead of <%c>.\n",
				part, partn);
			Error(14);
		}
...
void usage()
{
	format("usage: uudecode [-n] [-d] [-s dir] [-t dir] [input-file]\n");
	Error(1);
}
