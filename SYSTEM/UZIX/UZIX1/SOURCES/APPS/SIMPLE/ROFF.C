...
/* undefine this if your terminal can be controled by tcgetattr/tcsetattr */
#define NO_TERM_CONTROL
...
_PROTOTYPE(int titlen, (char *t, char c, int k));
_PROTOTYPE(void spits, (char *s));
_PROTOTYPE(void spit, (char c));
_PROTOTYPE(int suck, (void));
_PROTOTYPE(char *strhas, (char *p, char c));
...
char **argv;
{
  if (isatty(fileno(stdin)) && argc < 2) bomb();
...
void openmac()
{
  if (NULL == (Macwrite = fopen(tmpnam(mfilnam), "w"))) {
