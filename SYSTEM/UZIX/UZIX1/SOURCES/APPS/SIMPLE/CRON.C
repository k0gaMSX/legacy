...
static time_t prv_time;			/* timekeeper: previous wakeup	*/
...
void load_crontab()
{
  ...
  if (stat(CRONTAB, &buf)) {
	if (!started) fprintf(log, "Can't stat crontab\n");
	started = 0;
	return;
  }

  if ((convtime(&buf.st_mtime) - convtime(&prv_time)) <= 0) return;

  if ((cfp = fopen(CRONTAB, "r")) == NULL) {
	if (!started) fprintf(log, "Can't open crontab\n");
	started = 0;
	return;
  }
...
char *argv[];
{
  ...
  for (i = 1; i <= NSIGS; i++) signal(i, SIG_IGN);
