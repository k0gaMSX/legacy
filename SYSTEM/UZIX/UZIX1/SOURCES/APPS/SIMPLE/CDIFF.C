...
#define qtime(x)	asctime(localtime(x))
...
char **argv;
{
  ...
  if (argc != 2) {
	fprintf(stderr, "usage: cdiff -c# old new\n");
	exit(2);
  }
  old = argv[0];
  new = argv[1];
  ...
  if (!crcfp) {
	 ...
	 */
	fprintf(stderr, "Can't execute 'crc %s'.\nCheck if your system has the 'crc' program and if it's in the path.", old);
	exit(2);
  }
  ...
  printf("*** %s\tcrc=%s\t%s", old, sysbuf1, qtime(&statbuf.st_mtime));
  fstat(fileno(newfp), &statbuf);
  printf("--- %s\tcrc=%s\t%s", new, sysbuf2, qtime(&statbuf.st_mtime));
