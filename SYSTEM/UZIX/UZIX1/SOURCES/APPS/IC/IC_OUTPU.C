...
#if defined(_UZI_280_) || defined(PC_UZIX_TARGET)
#define PC
#endif
...
void Draw_Help_Screen()
{
...
#else
  printf("         l   Last top of stack\n");
#endif
  printf("         m   Pop shell                         .   Change sign\n");
  ...
  printf("         w");
#endif
  printf("   Write top to scratch pad          &   And\n");
  ...
  switch (output_base) {
	...
	break;
      ...
      case BINARY:{
		...
		for (mask = 0x8000000; mask > 1; mask >>= 1)
