...
static char RcsId[] = "$Header: archiver.c,v 1.23 91/06/20 14:22:37 ceriel Exp $";
...
char *argv[];
{
  ...
  if (local_fl) {
	char *s;
	s = strrchr(temp_arch, '/');
	if (s == NULL) s = temp_arch - 1;
	strcpy(temp_arch, s + 1);
  }
#ifdef AAL
...
register char *argv[];
{
  ...
  while ((member = get_member()) != NIL_MEM) {
	if (argc > 3) {
		for (i = 3; i < argc; i++) {
			if ((fnd = equal(basename(argv[i]), member->ar_name)))
				break;
		}
		if (!fnd || app_fl) {
			if (rep_fl || del_fl
