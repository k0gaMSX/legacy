 ...
 */
static void subcommand(cp, num1, num2)
	char   *cp;
	NUM	num1;
	NUM	num2;
{
	...
	*cp++ = '\0';
	...
	while (num1 <= num2) {
		...
		/* If the replacement string is the same size or shorter than the
		 * old string, then the substitution is easy. */
		if (deltalen <= 0) {
		...
		/* The new string is larger, so allocate a new line structure and
		 * use that.  Link it in in place of the old line structure. */
	...
	BOOL	expandflag;
{
	...
	while (!intflag && (num1 <= num2)) {
		...
		/* Show control characters and characters with the high bit set
		 * specially. */
		cp = lp->data;
		count = lp->len;
