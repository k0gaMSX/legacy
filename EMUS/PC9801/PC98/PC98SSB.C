...
/** for SoundBlaster16 for PC-9800                          **/
/**                                                         **/
...
dword OPL3Reg  = 0x20d2;          /* OPL3 I/O port             */
dword GameReg  = 0x04d2;          /* Game I/O port             */
...
void OPL3Out(dword r,byte v)
{
  ...
  reg=(r > 0xff)? OPL3Reg + 0x200 : OPL3Reg;
  outportb(reg, r & 0xff);
  for(i = 0; i < 2; i++) inportb(OPL3Reg);

  outportb(reg + 0x100, v);
  for(i = 0; i < 2; i++) inportb(OPL3Reg);
}
...
int OPL3Init()
{
  ...
  while(Env[i] != NULL) {
    if(sscanf(Env[i++], "BLASTER=A%x ", &OPL3Reg) == 1) {
      OPL3Reg |= 0x2000; break;
    }
  }
  GameReg = (OPL3Reg & 0x00ff) | 0x0400;
